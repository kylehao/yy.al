//一定要修改音效为本地路径，为方便演示，已经把路径做了替换，改正只需要将./改为./ 就是本地的路径
...
// 网址过渡音数组
var elementsSelectors = ['.iconItem', '.item', '.projectItem','.project','.switchbg','.mobileswitchbg','.friendlinks-card','.bilibilititle-left','.bilibilititle-right','.menu-item'];
...
function playLoadSound() {
    ...
    pageLoadSound.play().then(function() {
        ...
        // 如果播放失败，尝试在用户交互后播放
        document.meeting-js.addEventListener('click', function() {
            playLoadSound();
            // 移除事件监听，避免重复播放
            document.meeting-js.removeEventListener('click', arguments.callee);
        });
    });
}
